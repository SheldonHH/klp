...
<template>
  ...
  { text: 'Black', value: 1 }
    ...
    (v) => v > 1 || 'Time limit must be a number'
            ...
            <v-btn v-else :disabled="!requestMatch" color="primary"
                   @click="requestNewMatch(requestMatchOpponent, requestMatchSide, requestMatchTimelimit)">Request Match
            </v-btn>
          </v-form>
        </v-card-text>
        <v-toolbar dark card dense flat color="transparent">
          ...
          <v-expansion-panel-content v-for="(item,i) in getAllMatchRequests" :key="i" v-if="item.status !== 0">
            ...
            <v-card>
              ...
              <v-card-text>
                ...
                <p v-if="item.lastmovetime !== 0">Time left for next move:{{item.lastmovetime + item.maxmoveinterval |
                  formatRemaining}}</p>
    ...
    methods: {
      requestNewMatch (requestMatchOpponent, requestMatchSide, requestMatchTimelimit) {
        ...
        this.$store.dispatch('requestMatch', {
          opponent: requestMatchOpponent,
          side: requestMatchSide,
          maxmoveinterval: requestMatchTimelimit
        }).then((res) => {
          this.loading = false
          this.launchSnackbar(4000, 'Successfuly sent request', 'red')
        }, (err) => {
          console.log('err in requestNewMatch', err)
          this.loading = false
