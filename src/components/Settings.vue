...
<template>
  ...
  <v-layout row wrap>
    <v-flex xs4>
      <v-card class="addApi text-xs-center">
        <v-card-title primary-title>
          <h5 class="headline">Add API Endpoint</h5>
        </v-card-title>
        <v-form class="ma-4" v-model="addEndpointForm" ref="form" lazy-validation>
          <v-select v-bind:items="[
            ...
            ]" item-value="value" v-model="addEndpointProtocol" label="Protocol" single-line bottom dense
                    required></v-select>
          <v-text-field label="Domain/IP address" v-model="addEndpointIp" :rules="[(v) => !!v || 'URL is required']"
                        placeholder="endpointdomain.com || api-kylin.eoshenzhen.io || localhost" required></v-text-field>
          <v-text-field label="Port" v-model="addEndpointPort" :rules="[
              (v) => !!v || 'Port is required',
              (v) => v > 79 || 'Port must be a number bigger that 79'
            ]" placeholder="8888" required></v-text-field>
          <v-btn color="primary" v-if="!addEndpointLoading"
                 @click="addEndpoint(addEndpointProtocol, addEndpointIp, addEndpointPort)" :disabled="!addEndpointForm">
            Add
          </v-btn>
    ...
    <v-flex xs8>
      <v-data-table v-bind:headers="endpointListHeaders" :items="getEndpoints" no-data-text="No endpoints available"
        ...
        <template slot="items" slot-scope="props">
          ...
          <v-btn color="red" @click="removeEndpoint(props.item.url)">Remove</v-btn>
          <v-btn v-if="endpointLoading">
            <v-progress-circular indeterminate color="primary"></v-progress-circular>
          </v-btn>
          <v-btn v-else-if="!getCurrentEndpoint || getCurrentEndpoint.url !== props.item.url" color="primary"
                 @click="useEndpoint(props.item.url)">Use
          </v-btn>
          <v-btn v-else-if="getCurrentEndpoint && getCurrentEndpoint.url === props.item.url"
                 @click="disconnectEndpoint(props.item.url)">Disconnect
          </v-btn>
        </template>
      </v-data-table>
    </v-flex>
